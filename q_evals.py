...
#                output = self.basic_model.get_tensor(output_details['index'])
    ...
    def __eval_func__(self, cur_step=0, logs=None, eval_freq=1):
        ...
        # evaluate model result -> embedding vector
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--extract", default=True, help='Model last layer extract')
    parser.add_argument("-s", "--summary", default=False, help='Model Summary')
    ...
    if args.extract:
      layer = basic_model.layers[:-1]
      basic_model = keras.Model(basic_model.inputs, layer[-1].output)
    # keras.utils.plot_model(basic_model, 'a.png', show_shapes=True)
    if args.summary:
      basic_model.summary()
