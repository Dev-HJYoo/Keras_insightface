...
def batchnorm_with_activation(inputs, activation="relu", zero_gamma=False, name=""):
    ...
    if activation:
        ...
        if activation.lower() == "prelu":
            nn = quantize_annotate_layer(keras.layers.PReLU(shared_axes=[1, 2], alpha_initializer=tf.initializers.Constant(0.25), name=act_name), DefaultDenseQuantizeConfig())(nn)
        else:
