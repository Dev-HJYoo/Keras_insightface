...
def __init_model_from_name__(name, input_shape=(112, 112, 3), weights="imagenet", **kwargs):
    ...
    if name_lower == "mobilenet":
        ...
        from backbones import q_resnet  # MXNet insightface version resnet
        ...
        model_class = getattr(q_resnet, model_name)
...
#     `fix_gamma=True` in MXNet means `scale=False` in Keras
    embedding = quantize_annotate_layer(keras.layers.BatchNormalization(momentum=bn_momentum, epsilon=bn_epsilon, scale=scale,), q_resnet.DefaultBNQuantizeConfig())(nn)
...
class q_NormDense(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        super(q_NormDense, self).build(input_shape)
    ...
    def get_config(self):
        config = super(q_NormDense, self).get_config()
        config.update(
            {
                "units": self.units,
...
@keras.utils.register_keras_serializable(package="keras_insightface")
class q_NormDenseVPL(q_NormDense):
...
@keras.utils.register_keras_serializable(package="keras_insightface")
class q_AconC(keras.layers.Layer):
    """
    ...
    def __init__(self, p1=1, p2=0, beta=1, **kwargs):
        super(q_AconC, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(q_AconC, self).build(input_shape)
    ...
    def get_config(self):
        return super(q_AconC, self).get_config()
...
def convert_to_mixed_float16(model, convert_batch_norm=False):
    ...
    def do_convert_to_mixed_float16(layer):
        ...
        if isinstance(layer, InputLayer):
            return layer
        if isinstance(layer, q_NormDense):
            return layer
